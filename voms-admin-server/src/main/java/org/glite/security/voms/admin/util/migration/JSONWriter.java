...
public class JSONWriter {
  ...
  private void date(Date date, Method method) {
    ...
    DateFormat formatter = (json != null) && (json.format().length() > 0)
        ? new SimpleDateFormat(json.format()) : this.formatter;
  ...
  /**
   * If true, an Enum is serialized as a bean with a special property _name=name() as all as all
   * other properties defined within the enum.<br/>
   * If false, an Enum is serialized as a name=value pair (name=name())
   * 
   * @param enumAsBean true to serialize an enum as a bean instead of as a name=value pair
   *        (default=false)
   */
